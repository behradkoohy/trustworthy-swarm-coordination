...
class Agent(nn.Module):
    def __init__(self, num_actions):
        ...
        self.network = nn.Sequential(
            ...
            nn.ReLU(),
            self.layer_init(nn.Conv2d(64, 64, 2, stride=1)),
            nn.ReLU(),
            nn.Flatten(),
            self.layer_init(nn.Linear(100, 512)),
            nn.ReLU(),
        )
        self.actor = self.layer_init(nn.Linear(10, 4), std=0.01)
        self.critic = self.layer_init(nn.Linear(10, 1), std=1)
