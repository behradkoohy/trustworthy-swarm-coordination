...
class WorldEnv(ParallelEnv):
    def __init__(self, n_drones=3, n_humans=2, drone_locations = None, human_locations = [(2,6), (6,4)], targets = [(9,9)], reward_human=-20, reward_safe_zone=-5, reward_target=100, render_mode=None, max_x=10, max_y=10, max_timesteps=300):
    ...
    def initialise_grid(self,humans,targets):
        ...
        human_grid = get_start_point_OHE_from_locations(np.array(world_grid),humans)
        safe_grid = add_safe_zones(human_grid,np.array(world_grid))
        all_grids.append(human_grid)
        all_grids.append(safe_grid)
        
        #ADD THE TARGET OHE GRID
        all_grids.append(get_start_point_OHE_from_locations(np.array(world_grid),targets))
        ...
        if self.drone_init_locations != None:
            ...
            for i in range(self.n_drones):
                new_agent_grid,unavailable_locs = get_agent_start_point_OHE(np.array(world_grid),unavailable_locs)
                all_grids.append(new_agent_grid)
    ...
    def reset(self):
        ...
        self.agent_grid_mapping = {agent_id: i for agent_id, i in zip(self.agent_name_mapping.keys(), range(3,3+self.n_drones))} #DRONES START FROM 3 ONWARDS
